...
void Cube::drawSquare(DrawingPolygon& dpfile, int startingX, int startingY, int length, int index)
{
    //Left Vertical line
...
void Cube::uniteSquares(DrawingPolygon& dpfile, int startingX, int startingY, int length, int index)
{
    /*
    for (int i = 0; i < 4; ++i) {
        dpfile.points_to_draw[index].setX(startingX + length*(i && 1));
        dpfile.points_to_draw[index++].setY(startingY + length*(i && 1 << 1));
        dpfile.points_to_draw[index].setX(startingX+length/2 + length*(i && 1));
        dpfile.points_to_draw[index++].setY(startingY+length/2 + length*(i && 1 << 1));
    }*/

    dpfile.points_to_draw[index].setX(startingX);
    dpfile.points_to_draw[index++].setY(startingY);
    dpfile.points_to_draw[index].setX(startingX+length/2);
    dpfile.points_to_draw[index++].setY(startingY+length/2);

    dpfile.points_to_draw[index].setX(startingX+ length);
    dpfile.points_to_draw[index++].setY(startingY);
    dpfile.points_to_draw[index].setX(startingX+length/2 + length);
    dpfile.points_to_draw[index++].setY(startingY+length/2);

    dpfile.points_to_draw[index].setX(startingX);
    dpfile.points_to_draw[index++].setY(startingY + length);
    dpfile.points_to_draw[index].setX(startingX+length/2);
    dpfile.points_to_draw[index++].setY(startingY+length/2 + length);
...
void Cube::placePoints(DrawingPolygon &dpfile, int startX, int startY, int length)
{
    dpfile.points_number = 24;
    int index = 0;
    drawSquare(dpfile,startX,startY,length, index);
    index += 8;
    drawSquare(dpfile,startX+length/2,startY + length/2, length, index);
    index += 8;
    uniteSquares(dpfile,startX,startY,length,index);
}
...
void Cube::on_pushButton_clicked()
{
    ...
    placePoints(drawingpolygon, startX, startY, length);
...
void Cube::on_drawRotated_clicked()
{
    DrawingPolygon drawingpolygon;
    int startX = ui->X_StartingPoint->value();
    int startY = ui->Y_StartingPoint->value();
...
void Cube::on_drawScaled_clicked()
{
    DrawingPolygon drawingpolygon;
    int startX = ui->X_StartingPoint->value();
    int startY = ui->Y_StartingPoint->value();
...
void Cube::on_drawReflected_clicked()
{
